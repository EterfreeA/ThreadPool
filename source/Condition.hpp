...
public:
	...
	void enter() noexcept
	{
		_validity.store(true, std::memory_order::relaxed);
	}
...
bool Condition<_Size>::wait_for(const std::chrono::duration<_Rep, _Period>& _relative)
{
	std::unique_lock lock(_mutex);
	return valid() \
		and _condition.wait_for(lock, _relative) == std::cv_status::no_timeout;
}
	...
	_Predicate _predicate)
{
	...
	return _condition.wait_for(lock, _relative, \
		[this, &_predicate] { return not valid() or _predicate(); });
}
...
bool Condition<_Size>::wait_until(const std::chrono::time_point<_Clock, _Duration>& _absolute)
{
	std::unique_lock lock(_mutex);
	return valid() \
		and _condition.wait_until(lock, _absolute) == std::cv_status::no_timeout;
}
