...
{
	std::unique_lock lock(_mutex);
	return valid() and _condition.wait_for(lock, _relative) == std::cv_status::no_timeout;
}
...
{
	std::unique_lock lock(_mutex);
	return _condition.wait_for(lock, _relative, [this, &_predicate] { return not valid() or _predicate(); });
}
...
{
	std::unique_lock lock(_mutex);
	return valid() and _condition.wait_until(lock, _absolute) == std::cv_status::no_timeout;
}
...
bool Condition<_Size>::wait_until(const std::chrono::time_point<_Clock, _Duration>& _absolute, _Predicate _predicate)
{
	std::unique_lock lock(_mutex);
	while (valid() and not _predicate() \
		and _condition.wait_until(lock, _absolute) != std::cv_status::timeout);
	return _predicate();
}
